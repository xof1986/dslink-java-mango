...
public class ApiClient {
  ...
  public void setBuilder(MangoConn conn) {
    builder = new MangoBodyBuilder(conn);
  }

  public void setFolder (MangoFolder folder) {
    builder.setBuilder(folder);
  }
  ...
  */
  ...
  public <T> T deserialize(ClientResponse response, TypeRef returnType) throws ApiException {
    ...
    if (contentType.startsWith("application/json")) {
        T t = json.deserialize(body, returnType);
        Class<?> clazz = t.getClass();
        if (clazz.isArray()) {
          if (body.length() > 3) {
            Object[] objs = (Object[]) t;
            Object o = objs[0];
            build(o.getClass(), response, body, o);
          } else {
            throw new ApiException(400, "Unavailable data for the time period");
          }
        } else {
          build(clazz, response, body, t);
        }
        return t;
    } else {
  ...
  private void build(Class<?> clazz, ClientResponse response, String body, Object model) {
    ...
    for (Method m : methods) {
      if (m.isAnnotationPresent(JsonProperty.class)) {
        ...
        try {
          ...
          Method method = clazz.getMethod(m.getName().replace("get", "set"), ret);
          Class<?> type = method.getParameterTypes()[0];
          Object instance;
          switch (value) {
            case "statusCode":
              Class<? extends Enum> enumClass = type.asSubclass(Enum.class);
              instance = Enum.valueOf(enumClass, "_" + response.getStatus());
              break;
            case "headers":
              Constructor<?> con = type.getConstructor(Map.class);
              instance = con.newInstance(response.getHeaders());
              break;
            case "body":
              instance = builder.bodyBuilder(ret, body, node);
              break;
            default:
              throw new RuntimeException("Unknown value: " + value);
          }

          method.invoke(model, instance);
        } catch (Exception e) {
          e.printStackTrace();
          throw new RuntimeException(e);
        }
